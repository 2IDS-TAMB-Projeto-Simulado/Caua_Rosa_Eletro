<?php
    require_once "../model/model_doces.php";
    session_start();

    //CADASTRAR DOCE
    if(isset($_POST["cadastrar_doce"])){
        $doce = new Doce();
        $resultado = $doce->cadastrar_doce($_POST["marca"], $_POST["tipo"], $_POST["data_validade"],$_POST["formato"], $_POST["tamanho"], $_SESSION['usuario']["USU_ID"], );
        if($resultado){
            echo "<script>
    ...
    //EDITAR DOCE
    if(isset($_POST["editar_doces"])){

    $doce = new Doce();
    $resultado = $doce->editar_doce(
        $_POST["marca"],
        $_POST["tipo"],
        $_POST["data_validade"],
        $_POST["formato"],
